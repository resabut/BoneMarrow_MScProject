...
rna.list <- pbmclapply(
  1:nrow(meta), function(i) {
    ...
    # filter cell type
    rna <- subset(rna, subset= simplified_ct %in% c("Progenitor cells"))
...
saveRDS(rna.data, "rna_processed_progenitors.rds", compress = F)
```
...
```{r}
saveRDS(rna.data, "rna_annotated_progenitors.rds", compress = F)
```
plot
```{r}
