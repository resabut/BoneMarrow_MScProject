...
rna.list <- pbmclapply(
  1:nrow(meta), function(i) {
    ...
    if (ncol(rna) < 100) {
      print(paste0("Skipping ", meta$sampleId[i], " because there are only ", ncol(rna), " cell(s) left (>100 required)"))
      return(NULL)
    }
    # filter cell type -- only progenitor cells, if there are no progenitor cells, skip the sample
    if (length(which(rna$simplified_ct == "Progenitor cells")) < 100) {
      print(paste0("Skipping ", meta$sampleId[i], " because there are not enough progenitor cells (>100 required)"))
      return(NULL)
    } else { # at least 100 progenitor cells
...
Here they force at least 100 cells per sample and make sure that all the sample matrices have the same number of features.
I lower it to 100 per samples because of the progenitors.

```{r}
# at least 100 cells per sample
rna.list <- rna.list[which(unlist(lapply(rna.list, ncol)) >= 100)]
...
saveRDS(rna.data, "rna_processed_adult_progenitors.rds", compress = F)
```
...
```{r}
saveRDS(rna.data, "rna_annotated_adult_progenitors.rds", compress = F)
```
plot
```{r}
