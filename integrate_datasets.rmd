...
rna.list <- pbmclapply(
  1:nrow(meta), function(i) {
    ...
    # filter cell type -- only immature cells
    rna <- subset(rna, subset= simplified_ct %in% c("Progenitor cells", "Monocytes", "B cells", "Dendritic cells","Erythrocytes", "Granulocytes"))
...
saveRDS(rna.data, "rna_processed_immature.rds", compress = F)
```
...
```{r}
saveRDS(rna.data, "rna_annotated_immature.rds", compress = F)
```
plot
```{r}
