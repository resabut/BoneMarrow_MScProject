...
rna.list <- pbmclapply(
  1:nrow(meta), function(i) {
    ...
    if (length(which(rna$simplified_ct == "Progenitor cells")) < 50) {
      print(paste0("Skipping ", meta$sampleId[i], " because there are not enough progenitor cells (>50 required)"))
      return(NULL)
    ...
    # I remove HSC cells because they seem to be a mix of HSC and progenitors.
    if (length(which(rna$ct != "HSC")) < 50) {
      print(paste0("Skipping ", meta$sampleId[i], " because there are not enough cells (>50 required)"))
      return(NULL)
    } else { # at least 50  cells
      # subset cells
      print(paste0("Keeping ", meta$sampleId[i], " cells"))
      rna <- subset(rna, subset= ct != "HSC")
    }
    # pre-process
...
saveRDS(rna.data, "rna_processed_adult_progenitorsnoHSC50.rds", compress = F)
```
...
```{r}
saveRDS(rna.data, "rna_annotated_adult_progenitorsnoHSC50.rds", compress = F)
```
plot
```{r}
