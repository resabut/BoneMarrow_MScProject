...
rna.list <- pbmclapply(
  1:nrow(meta), function(i) {
    ...
    # just B cell lineage
    cells_to_keep <- c("HSC", "MPP", "CLP", "proB cell", "preB cell", "Naive B cell")
...
saveRDS(rna.data, "rna_processed_adult_Bcell_50.rds", compress = F)
```
...
```{r}
saveRDS(rna.data, "rna_annotated_adult_Bcell_50.rds", compress = F)
```
plot
```{r}
...
# plot pdf
```{r}
pdf("plot_Bcell.pdf", width = 15, height = 10)
p1
p2
p3
