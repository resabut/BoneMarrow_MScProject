mod_plotClusterMeanAndDiff <- function(testobj, cluster = testobj[["cluster"]], free.scale1 = TRUE, free.scale2 = FALSE) {
  ...
  pd <- lapply(1 : length(fit), function(i) {
    ...
    tmp <- melt(tmp)
    tmp[, 1] = testobj[["pseudotime"]][tmp[, 1]]
  ...
  p1 <- ggplot(data = pd) +
    geom_line(aes(x = pseudotime, y = populationFitClusterMean, color = type), size = 1) +
    theme_classic() + facet_wrap(`~`(cluster), nrow = length(unique(pd$cluster)), scales = a1) +
    theme(legend.position = "none") +
    ylab("Model-fitted expression patterns for groups") + xlab("Pseudotime")
  ...
  p2 <- ggplot(data = pd2) + geom_line(aes(x = pseudotime, y = covariateGroupDiff), size = 1) + theme_classic() + facet_wrap(`~`(cluster), nrow = length(unique(pd2$cluster)), scales = a2) + xlab("Pseudotime") + ylab("Group difference")
