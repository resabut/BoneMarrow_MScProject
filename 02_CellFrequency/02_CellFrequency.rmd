...
```{r load data}
setwd("/home/inf-38-2022/MScProject/02_CellFrequency/")
# rna.data <- readRDS("../01_Integration/immature_sex_annotated.rds")
rna.data <- readRDS("../01_Integration/data/DISCO/full_DISCO_sex_annotated.rds")
```

```{r}
`%!in%` <- Negate('%in%')
# subset only non foetal samples, unsorted
rna.data <- subset(rna.data, subset = project_id %!in% c("E-MTAB-9389", "GSE166895")) # foetal
rna.data <- subset(rna.data, subset = cell_sorting %in% c("", "Bone Marrow Mononuclear Cells", "MNCs")) # unsorted
```
...
t_nk_cells <- c("Naive CD8 T cell", "Effector/Memory CD4 T cell", "Naive CD4 T cell",
                "GZMK+ CD8 T cell", "Gamma-delta T cell", "IFN-activated T cell", "Treg", "GZMB+ CD8 T cell", "MAIT", "T/NK cell (cycling)", "CD16+ NK",
                "Tissue-resident NK cell", "CD56+ NK", "ILC")
...
cells_to_keep <- c("Naive B cell",  "preB cell", "preB cell (cycling)", "proB cell", "proB cell (cycling)",
                       "Early SOX4+ erythroblast", "Late hemoglobin+ erythroblast", "Fetal HBG+ erythrocyte", "Intermediate EPCAM+ erythroblast",
                       "S100A+ preNeutrophil (cycling)", "S100A+ preNeutrophil", "Metamyelocyte/Band neutrophil", "Promyelocyte", "Myelocyte",
                       "pre-pDC (lymphoid origin)", "pre-pDC (myeloid origin)", "cDC2", "cDC2 (cycling)", "cDC1", "pDC",
                       "Megakaryocyte", "Megakaryocyte progenitor",
                       "CLP", "CMP", "GMP", "MEP", "MPP", "CDP", "HSC"
                       )
meta.data %>%
        filter(ct %in% cells_to_keep) -> meta.data
propeller(clusters = meta.data$ct, sample = meta.data$sample, group = meta.data$Predicted_sex) -> prop.out
prop.out
...
```
```{r}
# plot umap coloured by log2 ratio
# add log2 ratio
propeller_asin.out$M_F_Log2Ratio <- log2(propeller_asin.out$PropMean.M / propeller_asin.out$PropMean.F)


library(grDevices)
colors <- colorRampPalette(c("red","gray","blue"))(20)
plotcol <- colors[cut(propeller_asin.out$M_F_Log2Ratio, breaks=20)]

plot1 <- DimPlot(subset(data, downsample=1000, subset = ct %in% rownames(propeller_asin.out)), group.by="ct") + NoLegend()
LabelClusters(plot1, id="ct", color=plotcol, size = 5, repel=T, max.overlaps=Inf) -> plot2
plot2
```
...
# add log2 ratio
propeller_asin.out$M_F_Log2Ratio <- log2(propeller_asin.out$PropMean.M / propeller_asin.out$PropMean.F)
propeller_asin.out$ct <- rownames(propeller_asin.out)
# add metadata
ct <- data@meta.data["ct"]
rownames(ct) <- ct$cell_id
ct %>%
  left_join(propeller_asin.out, by = c("ct" = "ct") )-> ct
# add
data[["log2MF"]] <- ct$M_F_Log2Ratio
DimPlot(subset(data, downsample=1000, subset = ct %in% rownames(propeller_asin.out)), group.by="log2MF") + NoLegend()

FeaturePlot(subset(data, downsample=1000, subset = ct %in% rownames(propeller_asin.out)), features = "log2MF", cols=c("#ff00ff",  "#ffff66"), blend=FALSE) -> plot1
LabelClusters(plot1, id="ident", color="black", size = 5, repel=T, max.overlaps=Inf) -> plot2
plot2
...
```{r}
`%!in%` <- Negate('%in%')
# subset at prject and sample level
rna.data <- subset(rna.data, subset = project_id %!in% c("E-MTAB-9389", "GSE166895")) # foetal
# rna.data <- subset(rna.data, subset = project_id %!in% c("E-MTAB-11536")) # samples from diseased patients
rna.data <- subset(rna.data, subset = cell_sorting %in% c("", "Bone Marrow Mononuclear Cells", "MNCs")) # unsorted2
rna.data <- subset(rna.data, subset = Predicted_sex != "inconclusive") # sex determined

```
```{r}
rna.data@meta.data %>%
        distinct(project_id, sample, age) %>%
        filter(project_id %in% c("HCA_HematopoieticImmuneCellAtlas", "GSE120221", "GSE185381")) %>%
# histogram age split by project_id
ggplot( aes(x = age, fill = project_id)) +
...
barplot(
  props$Proportions,
  col = hue_pal()(nrow(props$Proportions)),
  legend = TRUE,
  ylab = "Proportions"
)
...
```{r}
# remove the projects with only one individual -> introduce batch effects not balanaced by sex
data@meta.data %>%
distinct(sample, project_id) %>%
  group_by(project_id) %>%
  summarise(n = n()) %>%
    filter(n > 2)-> projects_to_keep
projects_to_keep
data <- subset(data, subset = project_id %in% projects_to_keep$project_id)
# keep only
# recalc proportions
...
fitting a linear model using the transformed proportion directly



```{r}
data <- AddMetaData(data, metadata = ifelse(data$age < 45, "young", "old"), col.name = "age_groups")
meta.data <- data@meta.data
...
sample_meta.data <- meta.data %>%
  distinct(Predicted_sex, sample, project_id, age_groups, age) %>%
...
early_progenitor_cells <- c("CMP", "HSC", "MPP")
stroma <- c("Fibroblast", "Mesenchymal stromal cell", "Osteoclast")
cells_to_keep <- c("Naive B cell",  "preB cell", "preB cell (cycling)", "proB cell", "proB cell (cycling)",              "Early SOX4+ erythroblast", "Late hemoglobin+ erythroblast", "Fetal HBG+ erythrocyte", "Intermediate EPCAM+ erythroblast","S100A+ preNeutrophil (cycling)", "S100A+ preNeutrophil", "Metamyelocyte/Band neutrophil", "Promyelocyte", "Myelocyte", "pre-pDC (lymphoid origin)", "pre-pDC (myeloid origin)", "cDC2", "cDC2 (cycling)", "cDC1", "pDC", "Megakaryocyte", "Megakaryocyte progenitor", "CLP", "CMP", "GMP", "MEP", "MPP", "CDP", "HSC")
```
```{r}

res$BaselineProp.clusters <- rownames(res)
res$PropRatioLog2 <- log2(res$PropRatio)
res$M_F_Log2Ratio <- log2(res$PropMean.Predicted_sexM / res$PropMean.Predicted_sexF)
res$sign <- ifelse(res$P.Value < 0.05, "less_0.05",
                                  ifelse(res$P.Value < 0.1, "less_0.1", "above_0.1"))
res$lineages <- ifelse(res$BaselineProp.clusters %in% lymphoid_lineage, "Lymphoid",
                                      ifelse(res$BaselineProp.clusters %in% gmp_lineage, "Granulocyte-Monocyte",
                                             ifelse(res$BaselineProp.clusters %in% mep_lineage, "Megakaryocyte-Erythroid",
                                                    ifelse(res$BaselineProp.clusters %in% dendritic_cell_lineage, "Dendritic",
                                                           ifelse(res$BaselineProp.clusters %in% early_progenitor_cells, "Early progenitors",
                                                                  ifelse(res$BaselineProp.clusters %in% stroma, "Stroma",
                                                                         "Others"))))))
res %>%
...
```{r}
# # umap colured by log change
# res$M_F_Log2Ratio <- log2(res$PropMean.Predicted_sexM / res$PropMean.Predicted_sexF)
#
# library(grDevices)
# library(RColorBrewer)
# colors <- colorRampPalette(brewer.pal(11,'Spectral')[-6])(ncol(data))
# plotcol <- colors[cut(res$M_F_Log2Ratio, breaks=ncol(data))]
# plot(Embeddings(data, "umap"), asp=1, pch=16, col=plotcol , las=1)
# LabelClusters(plot1, id="ident", color="black", size = 5, repel=T, max.overlaps=Inf) -> plot2
# plot2
...
# add log2 ratio
res$M_F_Log2Ratio <- log2(res$PropMean.Predicted_sexM / res$PropMean.Predicted_sexF)
# find ct where log2 ratio is not infinite, e.g. not detected in some sex
res$ct <- rownames(res)

res[!is.infinite(res$M_F_Log2Ratio),]$ct -> ct_to_show
# add metadata
ct <- data@meta.data["ct"]
rownames(ct) <- ct$cell_id
ct %>%
  left_join(res, by = c("ct" = "ct") )-> ct
# add
data[["log2MF"]] <- ct$M_F_Log2Ratio

Idents(data) <- data[["ct"]]
FeaturePlot(subset(data, subset = ct %in% ct_to_show ), features = "log2MF", cols=c("#ff00ff",  "#ffff66"), blend=FALSE, raster = FALSE) -> plot1
LabelClusters(plot1, id="ident", color="black", size = 5, repel=T, max.overlaps=Inf) -> plot2
# plot2
plot2 + paletteer::scale_colour_paletteer_c("ggthemes::Gold-Purple Diverging",
                                            direction =-1,
                                            limits = c(-3, 3)
                                            ) -> plot3
# plot3

plot2 + scale_color_gradient2(low = "#ff00ff", high = "#ffff66" ,
                             midpoint = 0, mid = "gray",
                             limits = c(-3,3)) -> plot4
plot4
plot4$data[["FDR"]] <- ct$FDR
...
sample_info <- meta.data %>%
  distinct(Predicted_sex, sample, project_id, age, age_groups) %>%
...
adonis.res
# make stacked bar plot with adonis.res$R2
