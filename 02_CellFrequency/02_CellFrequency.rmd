...
propeller_asin.out$sign <- ifelse(propeller_asin.out$P.Value < 0.05, "green",
                                  ifelse(propeller_asin.out$P.Value < 0.1, "yellow", rgb(1, 0, 0, 0.25)))
propeller_asin.out$lineages <- ifelse(propeller_asin.out$BaselineProp.clusters %in% b_cells, "B cell",
                                      ifelse(propeller_asin.out$BaselineProp.clusters %in% t_nk_cells, "T and NK cell",
                                             ifelse(propeller_asin.out$BaselineProp.clusters %in% granulocytes, "Granulocyte",
                                                    ifelse(propeller_asin.out$BaselineProp.clusters %in% monocytes, "Monocyte",
                                                           ifelse(propeller_asin.out$BaselineProp.clusters %in% erythrocytes, "Erythroid",
                                                                  ifelse(propeller_asin.out$BaselineProp.clusters %in% progenitor_cells, "Progenitor",
                                                                         ifelse(propeller_asin.out$BaselineProp.clusters %in% Mk, "Megakaryocitic",
...
propeller_asin.out %>%
        ...
        ggplot(aes(x = M_F_Log2Ratio, y = BaselineProp.clusters,
                   color = sign)) +
        scale_color_manual(values = c("green", "yellow", rgb(1, 0, 0, 0.25)),
                           labels = c("P < 0.05", "P < 0.1", "P > 0.1"),
                           breaks = c("green", "yellow", rgb(1, 0, 0, 0.25)),
                           name = "P value") +
        geom_point(
                # bigger point size
                size = 5
