...
I wrote this to add the info midway through the analysis, but it is better to add it at the beginning.
```
bm@meta.data %>%
  filter(project_id == "HCA_HematopoieticImmuneCellAtlas") %>%
  select("simplified_ct")-> simplified_ct_HCA

data <- AddMetaData(data, metadata = simplified_ct_HCA, col.name = "simplified_ct")
```
...
In this case, HCA data is chosen.
```{r choose dataset}
data <- bm.HCA
```
...
VlnPlot(data, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.ribo"),
        ...
        ncol = 2) +
        plot_annotation("QC metrics for one dataset") -> qc_plot
qc_plot
```
...
# save after scaling
saveRDS(data, file = "data/HCA/HCA_HematopoieticImmuneCellAtlas_scaled.rds", compress = FALSE)
```

### Load
```{r load, eval = FALSE}
# load
data <- readRDS(file = "data/HCA/HCA_HematopoieticImmuneCellAtlas_scaled.rds")
```
...
data %>%
        ...
        ggtitle("PCA coloured by sample_id")-> pca_plot
pca_plot
```
...
## Clustering
Based on the Elbow plot, we can see that the first 10 PCs explain most of the variance.
...
data %>%
  ...
  ggtitle("Simplified cell type frequencies per cluster")-> ct_seu_clusters
ct_seu_clusters
```
### confirm by sample
...
ggsave("data/HCA/HCA_HematopoieticImmuneCellAtlas_markers1_plot.png", markers1_plot, width = 11, height = 8.5)
ggsave("data/HCA/HCA_HematopoieticImmuneCellAtlas_markers2_plot.png", markers2_plot, width = 11, height = 8.5)
ggsave("data/HCA/HCA_HematopoieticImmuneCellAtlas_markers3_plot.png", markers3_plot, width = 11, height = 8.5)
ggsave("data/HCA/HCA_HematopoieticImmuneCellAtlas_markers4_plot.png", markers4_plot, width = 11, height = 8.5)
...
```{r save, eval = FALSE}
# save
saveRDS(data, file = "data/HCA/HCA_HematopoieticImmuneCellAtlas.rds", compress = FALSE)
```
...
pdf("data/HCA/HCA_HematopoieticImmuneCellAtlas_plots.pdf", width = 11, height = 8.5)
plot_list
dev.off()
```
