...
for (i in seq_along(simplified_ct_list$simplified_ct)) {
        ...
        data@meta.data %>%
                filter(age_groups == "under 45") %>% # only young ppl
                group_by(sample_id, gender, age, short_proj_id, ct, simplified_ct) %>%
                summarise(N = n()) %>%
                mutate(freq = N / sum(N)) %>%

                ungroup() %>%
                complete(sample_id, gender, age, short_proj_id,
                         fill = list(N = 0, freq = 0)) %>%
                ggplot(aes(x = gender, y = freq)) +
                geom_boxplot() +
                geom_point(aes(color = short_proj_id)) +
                geom_text(aes(label = age), size = 3, vjust = -0.5) +
                facet_wrap(~ct, scales = "free") +
                labs(title = paste0("Distribution per ", cell_type)) -> plot
