...
# see individuals
bm@meta.data %>%
  distinct(project_id, subject_id, sample_id, gender, age, cell_sorting) -> indiv.metadata
indiv.metadata
...
# plot QC metrics
VlnPlot(bm, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.ribo"),
        group.by= "project_id",
...
We can also detect the expression of Y-linked genes
...
# bm <- ScaleData(bm) # it needs to be scaled
ave_exp <- AverageExpression(bm, features = c(y_genes, x_fem_genes), group.by = "sample_id")[[1]] %>% t()  %>% as.data.frame() %>% arrange(desc(XIST))
bm$sample_id <- factor(bm$sample_id, levels = rownames(ave_exp))
Idents(bm) <- bm$sample_id
# DoHeatmap(subset(bm, downsample=10), features=c(y_genes, x_fem_genes),
#           group.by = "sample_id",
#           slot = "scale.data",
#           angle = 90) +
# #remove legend
# NoLegend() -> sex_genes_heatmap
# sex_genes_heatmap



combined_averages <- AverageExpression(bm,return.seurat = TRUE, assays = "RNA", features = c(y_genes, x_fem_genes), add.ident = "sample_id")
DoHeatmap(combined_averages, features=c(y_genes, x_fem_genes),
          slot = "scale.data",
...
# add age_groups metadata
# under and over 45
bm <- AddMetaData(bm, metadata = ifelse(bm$age < 45, "under 45", "over 45"), col.name = "age_groups")
# check
bm@meta.data %>% distinct(age_groups)
```
...
short_proj_id <- ifelse(bm$project_id == "HCA_HematopoieticImmuneCellAtlas", "HCA",
                        ifelse(bm$project_id == "GSE120221", "Oejten",
                              ifelse(bm$project_id == "GSE185381", "Nadorp-AML", "")))
bm <- AddMetaData(bm, metadata = short_proj_id, col.name = "short_proj_id")
```

## Save fixed object

```{r save fixed object}
saveRDS(bm, "data/DISCO/bmv2_fixed.rds", compress = FALSE)
```
...
simplified_ct_list <- data@meta.data %>% distinct(simplified_ct, ct)
...
for (i in seq_along(simplified_ct_list$simplified_ct %>% unique())) {
        ...
        #color.by= "short_proj_id",
                      max.normalize = TRUE,
                      vars.use = ct_list,
        main = paste0(cell_type, " frequency all ages")) -> plot
...
```{r}
pdf("data/unsorted/all_ages_individual_celltype_distribution.pdf", width = 10, height = 10)
...
for (i in seq_along(simplified_ct_list$simplified_ct %>% unique())) {
        ...
        #color.by= "short_proj_id",
                      max.normalize = TRUE,
                      vars.use = ct_list,
                      cells.use = rownames(data@meta.data)[which(data[["age_groups"]] == "under 45")],
        main = paste0(cell_type, " frequency under 45")) -> plot
...
This extracts the frequency data from dittoFreqPlot and plots the right frequencies with individual scales.
#### under 45
```{r}
...
for (i in seq_along(simplified_ct_list$simplified_ct %>% unique())) {
        ...
        #color.by= "short_proj_id",
                      ...
                      cells.use = rownames(data@meta.data)[which(data[["age_groups"]] == "under 45")],
                      data.out = TRUE,
        main = paste0(cell_type, " frequency under 45")) -> freq.data
        freq.data[[2]] %>%
