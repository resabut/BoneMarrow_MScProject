...
# set the file appendix
file_appendix <- "immature_hema_niche_50"
# load libraries
library(Seurat)
library(pbmcapply)
library(scDblFinder)
...
# # no cell sorting
# meta <- meta[which(is.na(meta$cellSorting)),]
# two big datasets
...
rna.list <- pbmclapply(
  1:nrow(meta), function(i) {
    ...
    if (ncol(rna) < 50) {
      print(paste0("Skipping ", meta$sampleId[i], " because there are only ", ncol(rna), " cell(s) left (>50 required)"))
      return(NULL)
    }
    # filter cell type -- only progenitor cells, if there are no progenitor cells, skip the sample
    #  remove the cells that are terminally differentiated or have left the hematopoietic niche (e.g. plasma cells, NK/ T cells, etc.)
    ...
    cells_to_keep <-
    if (length(which(rna$ct %in% cells_to_keep )) < 50) {
      print(paste0("Skipping ", meta$sampleId[i], " because there are not enough cells (>50 required)"))
      return(NULL)
    } else { # at least 50 progenitor cells
      # subset progenitor cells
      print(paste0("Keeping ", meta$sampleId[i], " cells"))
      rna <- subset(rna, subset= ct %in% cells_to_keep)
    }
    print(any(is.na(rna[["RNA"]]@data)))
    print(any(is.nan(rna[["RNA"]]@data)))
    print(any(is.infinite(rna[["RNA"]]@data)))
    print(any(is.null(rna[["RNA"]]@data)))
    # pre-process
...
Here they force at least 50 cells per sample and make sure that all the sample matrices have the same number of features.
I lower it to 50 per samples because of the progenitors.

```{r}
# at least 50 cells per sample
rna.list <- rna.list[which(unlist(lapply(rna.list, ncol)) >= 50)]
# # 33538 features in total
# rna.list <- rna.list[which(unlist(lapply(rna.list, nrow)) == 33538)]
#counts how many NA there are in a matrix
rna.list <- rna.list[which(unlist(lapply(rna.list, function(x) sum(is.na(x)))) == 0)]
```
Save the list
```{r}
...
saveRDS(rna.data, paste0("rna_processed_", file_appendix,".rds"), compress = F)
```
...
```{r}
saveRDS(rna.data, paste0("rna_annotated_", file_appendix,".rds"), compress = F)
```
plot
```{r}
...
# plot pdf
```{r}
pdf(paste0("plot", file_appendix, ".pdf"), width = 15, height = 10)
p1
p2
p3
